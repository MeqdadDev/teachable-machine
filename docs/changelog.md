...
- **Nested-Sequential `.h5` exports fail to load under Keras 3**: some Teachable Machine `.h5` exports save the model as a `Sequential` wrapping nested `Sequential`/`Functional` submodels. Keras 3's legacy H5 loader mis-rebuilds that shape (a `Functional` submodel's single output comes back as a length-1 list, which the next layer misreads as two inputs), previously surfacing as a misleading `FileNotFoundError: Model file not found`. `TeachableMachine` now falls back to a manual reconstruction that flattens the nested wrappers, rebuilds the graph via the Functional API, and restores weights by matching each layer's name against the saved weight paths.
- **Prediction annotation crashes on Windows / recent Pillow**: `show_prediction_on_image()` used `ImageFont.truetype("DejaVuSans-Bold.ttf", ...)`, not resolvable by bare name on Windows (`OSError`), and `ImageDraw.textsize()`, removed in Pillow 10+. Both are now handled: a `ImageFont.load_default()` fallback when the named font can't be found, and `textbbox()` in place of `textsize()`. Also fixed the highlight box behind the prediction label clipping text descenders (e.g. "g", "%") by measuring the box at the actual draw position instead of reusing an origin-measured extent.
- **Package build was broken**: `setup.py` reads `__version__` from `src/__init__.py`, but that file was never declared as package data, so it was missing from the sdist. Building the wheel from the sdist (as `pypa/gh-action-pypi-publish`'s two-phase build does) then failed with `FileNotFoundError`, before any PyPI upload was attempted. Added `MANIFEST.in` to include it.
